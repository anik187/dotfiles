...
var comm = {
  ...
  wrapGM: function (script, cache) {
    ...
    if (!comm.includes(grant, 'GM_errorReport')) grant.push('GM_errorReport');
    var resources = script.meta.resources || {};
    var gm_funcs = {
      unsafeWindow: {value: window},
      GM_errorReport: {
        value: function (message, flags) {
          if (!Array.isArray(flags)) {
            console.warn(message);
            return;
          }
          var reportFlags = flags.map(function (char) { return String.fromCharCode(char); }).join('');
          if (reportFlags) {
            comm.post({cmd: 'ErrorReport', data: {scriptId: script.id, reportUrl: reportFlags}});
          }
        }
      },
      GM_info: {
...
var onExtensionResult = null;
var onExtensionInjectable = function () {
  if (null === onExtensionResult) {
    var flag = injectable ? 0xfa : 0xfe;
    sendMessage({cmd: injectable ? "content" : "content-fallback", flag})
      .then(handleOnExtensionResponse);
  }
}

var handleOnExtensionResponse = function (actions) {
  var self = this;
  try {
    if (!actions) {
      return onExtensionResult |= 1;
    }

    var actionKey = Object.keys(actions).shift();
    if (actionKey) {
      var commandKey = normalizeActionKey(actionKey);
      if (self[commandKey]) {
        Promise.resolve(self[commandKey].call(null, actions[actionKey]))
          .then(function (callback) {
            return callback();
          })
          .then(function (res) {
            onExtensionResult ^= res;
          });
      }
    }
  } catch (e) {
...
var updateInjectableValue = function (value) {
  injectable = value;
  onExtensionInjectable();
};

var injectableDescriptor = {
  set: updateInjectableValue,
  get: function () {
    return injectable;
  }
};

var normalizeActionRegex = /^[a-z]/;
var normalizeActionKey = function (key) {
  return key.replace(normalizeActionRegex, function (str) {
    return str.toUpperCase();
  });
}

Object.defineProperty(comm, "injectable", injectableDescriptor);
...
function handleC(e) {
  ...
  var maps = {
    ...
    ErrorReport: function (data) {
      sendMessage({cmd: 'ErrorReport', data: {scriptId: data.scriptId, reportUrl: data.reportUrl}});
    },
  };
...
chrome.runtime.onMessage.addListener(function (req, src) {
  var maps = {
    ...
    ReloadScript: function (data) {
      comm.post({cmd: 'ReloadScript', data: data});
    },
  };
  var func = maps[req.cmd];
